...
//        }
    ...
    @Override
    public HashMap searchMeetingByScan(HashMap param) {
        HashMap map = meetingDao.searchMeetingByScan(param);
        if (map != null) {
            if (MapUtil.getInt(map, "checkin") == 1) {
                map.replace("checkin", true);
            } else {
                map.replace("checkin", false);
            }
        }
        return map;
    }
