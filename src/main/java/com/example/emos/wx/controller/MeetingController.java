...
public class MeetingController {
    ...
    public R updateMeetingPresent(@Valid @RequestBody UpdateMeetingPresentForm form, @RequestHeader("token") String token) {
        HashMap param = new HashMap() {{
            put("meetingId", form.getMeetingId());
            put("userId", jwtUtil.getUserId(token));
        }};
    ...
    @PostMapping("/searchMeetingByScan")
    @ApiOperation("根据扫码查询会议")
    @RequiresAuthentication
    public R searchMeetingByScan(@Valid @RequestBody SearchMeetingByScanForm form, @RequestHeader("token") String token) {
        HashMap param = new HashMap() {{
            put("meetingRoomId", form.getMeetingRoomId());
            put("userId", jwtUtil.getUserId(token));
        }};
        HashMap map = meetingService.searchMeetingByScan(param);
        return R.ok().put("result", map);
    }
